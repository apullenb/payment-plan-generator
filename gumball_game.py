def play_game(box_ids, goals, num_jars, min_gumballs, gumballs_per_jar):
    num_boxes = len(box_ids)
    total_rounds = 0

    # Initialize the boxes with the minimum number of gumballs
    boxes = {box_id: min_gumballs for box_id in box_ids}
...
min_gumballs = int(input("Enter the minimum number of gumballs for each box: "))
gumballs_per_jar = int(input("Enter the number of gumballs per jar: "))

# Play the game
total_rounds = play_game(box_ids, goals, num_jars, min_gumballs, gumballs_per_jar)
